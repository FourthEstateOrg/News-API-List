# Contributing to News API List

> While pull requests and community involvement are appreciated, some pull requests have been specifically
opened to market company APIs that offer third party or aggregate solutions. The News API list is not a marketing tool, but a tool to help the news and journalism
community build applications powered by legitimate and trusted news organizations and services. Pull requests that are identified as marketing attempts will not be accepted.
>
> Please make sure the API you want to include is from a news organization or service.  An example that would be rejected is an API that is from a service that scrapes or aggregates content from news agencies, and doesn't produce any content themselves.
>
> Thanks! :)
...
```
| [News API Name](https://domain.com) | Short description of the news api | No | Yes | Yes |
```
...
* The Description should not exceed 280 characters.
* Make sure the PR title is in the format of `Add Api-name API` *for e.g.*: `Add News API List`
* Use a short descriptive commit message. *for e.g.*: ❌`Update Readme.md`  ✔ `Add API NAME to News API List`
